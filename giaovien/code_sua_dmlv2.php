	...
	<main>
		...
		<?php if(isset($_GET['id'])){
			if (!$conn) {
				...
				$sql = "select * from table_danhmuc_lv2 where id = $id";
				$list = $conn->query($sql);
		...
		if(!empty($_POST['btnUpdate'])){
			...
			$sql = "update table_danhmuc_lv2 set ";
			echo $photo;

			if(($ten == $value['ten']) && ($mota == $value['mota']) && (empty($photo))){
				?>
				<script>
			...
			<section class="info_profile">
				<form method="POST" enctype="multipart/form-data">
					<div class="wrap_baigiang">
						<label id="lv1" >Tên danh mục cấp 2</label>
						<input type="text" name="ten" value="<?= $value['ten'] ?>">
					</div>
					<div class="wrap_baigiang">
						<label id="lv1">Mô tả danh mục cấp 2</label>
						<input type="text" name="mota" value="<?= $value['mota'] ?>">
					</div>
					<div class="wrap_baigiang">
						<label>Hình ảnh hiện tại:</label><br>
						<img src="../upload/danhmuclv2/<?= $value['photo'] ?>"><br>
