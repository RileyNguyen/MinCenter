	...
	<main>
		...
		<?php if(isset($_GET['id'])){
			if (!$conn) {
				...
				$sql = "select * from table_tieuchi where id = $id";
				$list = $conn->query($sql);
		...
		if(!empty($_POST['btnUpdate'])){
			...
			$sql = "update table_tieuchi set ";
			echo $photo;

			if(($ten == $value['ten']) && ($mota == $value['mota']) && ($noidung == $value['noidung']) && (empty($photo))){
				?>
				<script>
			...
			<section class="info_profile">
				<form method="POST" enctype="multipart/form-data">
					...
					<div class="wrap_baigiang">
						...
						<br>
						<img src="../upload/tieuchi/<?= $value['photo'] ?>">
						<br>
						<label>Chọn ảnh thay đổi:</label>
